...
- **Open (next steps of #2):** gauntlet respawn on WIN — owned by the controller (advance `Encounter`, free+respawn monster, `start()` next fight; player persists); else run-clear flow. Then retire the legacy `player_vbox` bridge (#6) and the dead `anti_operator`/`player_attack`/`monster_attack`. Per-round player dice reroll is also unhooked (controller rolls only at `_ready`) — decide if rounds should reroll.
