...
## 2026-06-25 (Code Claude — FSM rework groundwork: lean player entity + death-model design)
...
- **Open (next steps of #2):** route every FSM state through `_advance()` (reads `Combatants.player/monster.hp`) + delete `CHECK_DEFEAT`; migrate resolution from the mutating `anti_operator`/`player_attack` path to pure `compute_outcome`; repoint WIN/LOSE handlers off the legacy `monster_spawner` group to the rework spawn.
