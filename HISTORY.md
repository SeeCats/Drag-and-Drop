...
- **Deferred by user:** the rotate animation (ui-spec §5 design debt: dice slide one column, wrap die arcs over the row). Plan discussed (FLIP via the dice `top_level` trick + parabolic arc); build when called.
...
- **Open threads:** verify in-editor that dice.tscn + damage_number_rich_text_label.tscn load clean and drags feel right; still unfixed from the review — RunLog loses cleared runs quit within the 1s victory beat; consider a CI check that regenerates the sim's gauntlet table from the `.tres` files (this desync was invisible to eyeballs).
