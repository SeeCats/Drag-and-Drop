...
Mode-variation is an asset, not a leak: it gives the game texture and rests the "which color" muscle while working the "optimize and gamble" one (§7.4). **Corollary authoring rule: a monster needs anti for the player's BLUE/strip to mean anything** — race monsters correctly have none. Three guardrails keep modes healthy: (1) **situational, never dominant** — if number-stacking beats the color game against *most* monsters, the knot (the differentiator) is being bypassed; **watch this specifically in playtest**. Scope is the whole distinction: an obvious best move in a *single* fight is fine — that's the §7.4 brain-rest beat — what's forbidden is a line that stays dominant *across* fights and bypasses the read. **A fight you can autopilot is pacing; a game you can autopilot is a balance failure.** (2) **real downside** — a race is only a decision if whiffing the kill hurts (the Alien's spike does this job); (3) **legible** — "its guard is 0, just race it" should be a *read the player makes*, not a hidden trap; lean on full information and the lookahead.
...
**No rest turns is the core fatigue risk.** With no default pass button (see below) and a forced action every turn, there are none of the "obvious" autopilot turns most turn-based games use to let the brain coast. Fix it through **encounter intensity pacing**: not every monster is a hard knot — design plenty of fights with an obvious dominant move (brain rest — dominant *within* a fight only; a line dominant *across* fights is the §6.2 degeneracy, not rest) and spike intensity on elites/bosses. Compose a fatigue rhythm the way a level designer alternates tense and loose rooms. Dice variance gives some of this for free; lean into it deliberately.
...
### 7.9 The optimization line — requests we grant and refuse

Optimization can soak the fun out of a deep game, and players push for it through reasonable-sounding requests ("just sort my options," "let me see the reroll," "auto-resolve the easy ones"). Each is locally helpful and globally corrosive. This is the doctrine for which to grant and which to refuse. It governs the **interface / feature axis** of over-optimization; the **emergent-dominant-strategy axis** — a single line like race-everything that bypasses the read — is §6.2's job, countered situationally and never by removing the option.

**The bright line: the UI answers "what *would* happen?", never "what *should* I do?"** Render every outcome; recommend none. No rankings, sorts, suggested moves, auto-arrange, or win-%. The subtle part is the crux: **the ordering is the danger, not the data.** A player sweeping the knob through candidate outcomes is *play*; an enumerated, *sorted* list of those same outcomes is *judgment-removal*. Same numbers, opposite verdict — so the test is never "does this show information?" but "does this rank the choice for them?"

**Triage any request by the kind of effort it removes:**

- **Arithmetic / mechanical → always grant.** Offloading computation is a *precondition* of the loop (§1.2, §7.4), not a concession — show products, damage-after-reduction, thresholds.
- **Judgment → always refuse.** Anything answering "which move is best": rankings, suggested arrangements, win-%, auto-arrange.
- **Risk → refuse *free* removal.** No reroll previews, no undo-past-reveal, no pity timers. (Designer-side variance shaping and progression-*purchased* control stay legal — the player may *earn* safety, never be handed it for free; §7.4, §7.5.)
- **Rhythm / pacing friction → investigate, never grant directly.** "Auto-resolve the easy fights" is a *diagnostic*, not a feature request: the fight is either a working rest beat (decline — the lightness is intentional, §7.4) or dead content (fix the fight, don't skip it).

**Boundary example — "kill on N+" is legal.** It states a *fact* (you reach lethal at N), not a *recommendation* (you should race) — it tells the player what would happen and leaves the should-I-go-for-it decision entirely theirs. Sitting so close to the line is what makes it the useful test case (and its own tuning problem — §7.10).

**Meta-rule: take the location, discard the prescription.** Players are excellent reporters of *where* it hurts and unreliable prescribers of *what* to change (wave-1: "swap feels like a draw" was the real signal; the fix was threshold legibility, not the implied "make swap safe"). **Watch-list:** deeper lookahead (§5.2.G — one step is rationed on purpose) and "just tell me what to do" (the purest judgment-removal, easiest to grant by accident through over-helpful onboarding).

### 7.10 Kill-threshold legibility (a tuning target)

When the UI shows a kill threshold ("kill on N+", §7.9), that N *is* the kill probability made visible. On a d6 reroll: N=2 → 83%, N=3 → 67%, N=4 → 50%, N=5 → 33%. The push-your-luck sweet spot is the band **N ∈ [2, 4]** (50–83%) — "I can usually do it, but it's a real gamble," where the 제발 lives. Below 2 (kill on 1+) the gamble is dead (a free kill); above ~4–5 it stops reading as "go for it" and starts reading as RNG mugging you.

**N is not a knob.** Because damage is multiplicative, the threshold is emergent — roughly `HP_remaining ÷ partner_factor` — so it swings across the whole range with the arrangement and how chipped the monster is (`6×6 = 36` vs `2×2 = 4`). You cannot *set* N; you can only shape its **distribution**, via the three upstream levers §7.8 already names: the die range, monster HP and chip-curve, and the pattern values.

**Target:** *when a kill threshold is shown at all, it lands in ~[2, 4] for ~95% of those cases.* The condition matters — this applies to **race-eligible** turns only. On a turtle turn there is no kill to reach, and the UI showing *no* threshold is correct and legible ("this isn't a race turn"). Forcing every turn into the band would delete turtle turns, which we want.

**Sim to verify (don't guess the number, §7.8):** across the gauntlet's race-eligible decision points, measure the distribution of N, and test whether compressing the die range (2–6 / 3–6) and flooring mult at 2 pulls ~95% of it into [2, 4]. Queued for Code Claude.
