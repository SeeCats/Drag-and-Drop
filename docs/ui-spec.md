...
**INPUT REWORK 2026-07-06 (user + Design call) — supersedes the flick gestures below:** **swap = tap-tap** (tap a die → steady bright border marks it, candidates pulse; tap another → they exchange, the first-tapped lands there and rerolls on commit; tap the same die again → deselect) · **rotate = drag a die onto any slot** (the row cycles so the dragged die lands where dropped; the drop point feeds the settle animation; a press becomes a drag past ~12px) · **knob = display-only** (deal + HP ring keep their home; no input). The 2026-06-11 flick validation no longer covers input — both new gestures need the same one-handed divided-attention revalidation (wave-2). Bullets below describing flick staging, the turnable-knob affordance (notch/ticks), and §8 criteria 9–10 are **pending a reconciliation sweep** — read them as historical until swept.
...
- **The wrap animation (built 2026-07-06, was design debt).** On rotate, the end dice must travel to the row's other end. It must visibly arc over the row (not teleport) or rotate reads as chaos. Built as a FLIP flight: dice slide one column from their value's source slot, the wrapper arcs over the row (`Dice.fly_from` + controller `_animate_rotate`; see CLAUDE.md). The wrap path is animation only — never drawn as standing UI (it costs a band of dead space; learned from mockup).
...
7. ~~Knob discoverability = wave-2 criterion~~ RESOLVED BY SUPERSESSION 2026-07-06: the flick is retired before wave-2 ever tested it (§5 input rework — tap-tap promoted for swap, drag became rotate). Replacement wave-2 question: do untutored players discover *tap-tap = swap* and *drag = rotate*, and does drag-for-rotate fight the drag-for-swap convention they bring from other games?
...
- 2026-07-06 — §5 INPUT REWORK (Code, user + Design call): swap = tap-tap (first-tapped rerolls), rotate = drag a die onto any slot, knob = display-only. Flick retired untested-by-wave-2; open item 7 resolved by supersession; flick-dependent bullets + §8 criteria 9–10 flagged for a reconciliation sweep. New gestures need on-device revalidation.
- 2026-07-06 — §5 wrap animation built (Code, user-directed): rotate now animates as a FLIP flight — dice slide one column, the wrapper arcs over the row. Debt line rewritten as as-built.
